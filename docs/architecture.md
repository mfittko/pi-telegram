...
- `preview` / `replies` / `rendering` / `async-notify`: preview lifecycle/transports, final reply delivery and reply parameters, Telegram HTML Markdown rendering, chunking, stable-preview snapshots, and attributable async-run follow-up notices for Telegram-started async work.
