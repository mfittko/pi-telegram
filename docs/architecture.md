...
- `preview` / `replies` / `rendering` / `async-notify`: preview lifecycle/transports, final reply delivery and reply parameters, Telegram HTML Markdown rendering, chunking, stable-preview snapshots, and Telegram attribution for exact no-turn parent follow-up mirroring after Telegram-started async runs.
